...
import numpy as np
class UnimodalNet_absdistance(OrdinalLoss):
    def __init__(self, K, lamda=1.0):
        super().__init__(K)
        self.lamda = lamda

    def forward(self, ypred, ytrue):
        activation_pred = self.activation(ypred)
        ce_loss = ce(activation_pred, ytrue)
        pred_class = torch.argmax(activation_pred, dim=1)
        distance_loss = self.lamda * abs(pred_class - ytrue).float().mean()
        #print ('distance_loss', distance_loss.item()) #distance_loss 27.28125
        distance_loss = F.softmax(distance_loss) * ce_loss * 2 #
